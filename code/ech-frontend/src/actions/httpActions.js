...
export function getFilteredProjects(filters, currentPage, shouldConcatResults, sortBy) {
    return function (dispatch) {
        ...
        const options = {
            method: 'GET',
            url: getEndpoint('/api/get/projects'),
            params: {
                filters,
                resultsToSkip,
                itemsPerLoad,
...
export function getFilterOptions() {
    return function (dispatch) {
        const options = {
            method: 'GET',
            url: getEndpoint('/api/get/filteroptions'),
            params: {
                // filters,
                // resultsToSkip,
                // itemsPerLoad,
...
export function getSearchResults(searchTerm, currentPage, shouldConcatResults) {
    return function (dispatch) {
        ...
        const options = {
            method: 'GET',
            url: getEndpoint('/api/get/searchResults'),
            params: {
                searchTerm,
                resultsToSkip,
                itemsPerLoad
        ...
        axios(options)
            .then(response => dispatch(getSearchResults_SUCCESS({ projects: response.data.projects, totalResultsLength: response.data.totalResultsLength, shouldConcatResults, itemsPerLoad })))
...
export function sendNewProjectToBackend(projectData, token) {
    return function (dispatch) {
        ...
        const options = {
            ...
            url: getEndpoint('/api/create/project'),
            data: {
                projectData: {
                    ...projectData,
...
export function getInfoFromGitRepo(url) {
    return function (dispatch) {
        const options = {
            method: 'GET',
            url: getEndpoint('/api/get/project/git'),
            params: { url }
        }
...
export function getProjectByName(projectName) {
    return function (dispatch) {
        ...
        const options = {
            method: 'GET',
            url: getEndpoint('/api/get/project'),
            params: { projectName }
        }
...
export function requestLoginToken(mail, password) {
    return function (dispatch) {
        const options = {
            method: 'POST',
            url: getEndpoint('/api/create/token'),
            data: {
                mail: mail,
                password: password,
            }
...
export function registerUser(username, password, mail, name, organization, profileImageFile) {
    return function (dispatch) {
        ...
        const options = {
            ...
            data: formData,
            url: getEndpoint('/api/create/user')
        }
...
export function activateUser(activationToken) {
    return function (dispatch) {
        const options = {
            method: 'GET',
            url: getEndpoint('/api/activate/token'),
            params: {
                activationToken
            }
        }
